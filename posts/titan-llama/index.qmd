...
::: {.callout-warning}
This work was submitted as our final project for Deep Learning at MIT (6.7960). Many of the results are still preliminary as work is ongoing.
:::
...
::: {.callout-note}
Our prototype builds on an existing open-source Titans implementation that prioritizes correctness over performance. The neural memory kernels are therefore under-optimized, introducing overhead that causes throughput to lag slightly behind full attention in practice. We consider this an engineering issue and expect substantial speedups from better implementation.
:::
...
**Memory and state.** RNNs and LSTMs compress history into a fixed-size hidden vector but eventually forget distant information and cannot be parallelized. SSMs [@gu2021efficiently] revisit this idea by improving stability and parallelizability. Transformers on the other hand store the entire sequence explicitly, relying on an attention "state" that grows quadratically with sequence length. For the record, we think it's totally understated how extremely parallelizable and expressive transformers are--but mitigating memory consumption is still a key concern.

**Parameter-efficient adapters.** Full fine-tuning is expensive and risks catastrophic forgetting. LoRA [@hu2021lora] instead adds trainable low-rank factors to frozen weight matrices, which reduces the trainable parameters by orders of magnitude while preserving expressivity. Prefix-tuning [@li2021prefix] prepends learned continuous prompts to the input sequence, allowing the model to behave as if conditioned on specific tasks or domains while keeping the backbone frozen.

### Titans: Deep Neural Memory as State

**Titans: deep neural memory as state.** Titans [@behrouz2024titans] combines recurrent architectures as long-term memory with localized attention windows as short-term memory. These were the three things that we thought were the most important to understand from the paper. 
...
```python
# X: [b, d]   (tokens in a chunk)
# W0: [d, d]
# A: [b, b]   (A = Θ_b B_b)

# residuals r_i = (W0 x_i - x_i)
R = X @ W0.T - X  # [b, d]

# per-token outer products g_i = r_i x_i^T   -> [b, d, d]
G = torch.einsum('bi,bj->bij', R, X)  # [b, d, d]

# weighted prefix sums for every t in {1..b}:  U_t = Σ_i A[t,i] * G_i  -> [b, d, d]
U = torch.einsum('ti,ijk->tjk', A, G) # [b, d, d]

# if you only need the "end of chunk" update (t=b), grab:
dW_end = U[-1] # [d, d]
...
::: {.callout-note}
Note that **only Winogrande and BoolQ were properly hyperparameter-tuned**; for the other tasks we only ran ~30 training steps^[because we were running late to formal], so their scores are very under-optimized.
:::
